...
func (l Logger) AnalysisPublished(ctx context.Context, actor, analysisID int64, request string, now time.Time) error {
	return l.Record(ctx, domain.AuditEvent{ActorID: actor, EntityType: "analysis", EntityID: analysisID, Action: "analysis_published", Outcome: "ok", RequestID: request, CreatedAt: now})
}
